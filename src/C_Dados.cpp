...
void Dados::calcular_produtibilidade_para_cortes_NEWAVE(const double a_percentual_volume_util) {
	...
	try {
		...
		for (IdHidreletrica idHidreletrica = IdHidreletrica_1; idHidreletrica <= maiorIdHidreletrica; idHidreletrica++) {
			...
			const TipoPerdaHidraulica tipo_perda_hidraulica = getAtributo(idHidreletrica, AttComumHidreletrica_tipo_de_perda_hidraulica, TipoPerdaHidraulica());
			const double perda_hidraulica = getAtributo(idHidreletrica, AttComumHidreletrica_perda_hidraulica, double());
			const double fator_de_producao = getAtributo(idHidreletrica, AttComumHidreletrica_fator_de_producao, double());
			...
			double volume_minimo = getAtributo(idHidreletrica, IdReservatorio_1, AttComumReservatorio_volume_minimo, double());
			double volume_maximo = getAtributo(idHidreletrica, IdReservatorio_1, AttComumReservatorio_volume_maximo, double());
			...
			if getAtributo(idHidreletrica, AttComumHidreletrica_tipo_regularizacao, TipoRegularizacao()) == TipoRegularizacao_semanal || getAtributo(idHidreletrica, AttComumHidreletrica_tipo_regularizacao, TipoRegularizacao()) == TipoRegularizacao_diaria {
				volume_minimo = getAtributo(idHidreletrica, AttComumHidreletrica_volume_referencia, double());
				volume_maximo = getAtributo(idHidreletrica, AttComumHidreletrica_volume_referencia, double());
			...
			for (Periodo periodo = a_horizonte_estudo.getIteradorInicial(); periodo <= a_horizonte_estudo.getIteradorFinal(); a_horizonte_estudo.incrementarIterador(periodo)) {
				...
				const double canal_fuga_medio = getElementoVetor(a_idHidreletrica, AttVetorHidreletrica_canal_fuga_medio, periodo, double());
				const double cotaMedia = vetorHidreletrica.att(idHidreletrica).vetorReservatorio.att(IdReservatorio_1).getCotaMedia(periodo, volume_minimo, volume_maximo)
				const double cota = cotaMedia - canal_fuga_medio;

				if a_percentual_volume_util == 1.0 {
					vetorHidreletrica.att(idHidreletrica).vetorReservatorio.att(IdReservatorio_1).addElemento(AttVetorReservatorio_cota_referencia_EAR, periodo, cotaMedia);
					vetorHidreletrica.att(idHidreletrica).addElemento(AttVetorHidreletrica_produtibilidade_EAR, periodo, vetorHidreletrica.att(idHidreletrica).calcularProdutibilidade(tipo_perda_hidraulica, perda_hidraulica, fator_de_producao, cota));
				}//if a_percentual_volume_util == 1.0 {
				else if a_percentual_volume_util == 0.65 {
					vetorHidreletrica.att(idHidreletrica).vetorReservatorio.att(IdReservatorio_1).addElemento(AttVetorReservatorio_cota_referencia_ENA, periodo, cotaMedia);
					vetorHidreletrica.att(idHidreletrica).addElemento(AttVetorHidreletrica_produtibilidade_ENA, periodo, vetorHidreletrica.att(idHidreletrica).calcularProdutibilidade(tipo_perda_hidraulica, perda_hidraulica, fator_de_producao, cota));
				}//if a_percentual_volume_util == 0.65 {
				else
					throw std::invalid_argument("Nao identificado percentual_volume_util: " + std::string(a_percentual_volume_util));
	...
	catch (const std::exception& erro) { throw std::invalid_argument("Dados::calcular_produtibilidade_para_cortes_NEWAVE(): \n" + std::string(erro.what())); }
