...
void MetodoSolucao::executarPDDE(EntradaSaidaDados a_entradaSaidaDados, const IdProcesso a_idProcesso, const IdProcesso a_maiorIdProcesso, ModeloOtimizacao &a_modeloOtimizacao, Dados& a_dados){
	...
	try {
		...
		for (IdIteracao idIteracao = iteracao_inicial; idIteracao <= iteracao_final; idIteracao++) {
			...
			encerrar_iteracoes = executarPDDE_avaliarSolucao(a_entradaSaidaDados, idIteracao, a_idProcesso, a_modeloOtimizacao);

			if ((encerrar_iteracoes) && (iteracao_numero_maximo == iteracao_final))
				break;
			else if ((encerrar_iteracoes) && (idIteracao < iteracao_final))
				idIteracao = IdIteracao(iteracao_final - 1);
...
void MetodoSolucao::executarPDDE_forward(EntradaSaidaDados a_entradaSaidaDados, const IdEstagio a_estagio_inicial, const IdEstagio a_estagio_final, const bool a_simulacao, const IdIteracao a_idIteracao, const IdProcesso a_idProcesso, const IdProcesso a_maiorIdProcesso, ModeloOtimizacao &a_modeloOtimizacao, Dados& a_dados){
	...
	try {
		...
		std::string diretorio_iteracao = diretorio + "//" + getFullString(a_idIteracao) + "//Forward";
		
		if (a_simulacao) {
			const IdIteracao iteracao_final = a_modeloOtimizacao.arranjoResolucao.getAtributo(AttComumArranjoResolucao_iteracao_final, IdIteracao());
			const IdIteracao iteracao_numero_maximo = a_modeloOtimizacao.arranjoResolucao.getAtributo(AttComumArranjoResolucao_iteracao_numero_maximo, IdIteracao());
			if (iteracao_final == IdIteracao_0)
				diretorio_iteracao = diretorio;
			else if ((a_idIteracao == iteracao_final) && (iteracao_numero_maximo < iteracao_final))
				diretorio_iteracao = diretorio + "//Simulacao";
		}
		...
		IdEstagio idEstagio_final_custo_superior = a_estagio_final;
		if (!a_simulacao)
			idEstagio_final_custo_superior--;

		SmartEnupla<IdEstagio, std::vector<double>> custo_superior(a_estagio_inicial, std::vector<std::vector<double>>(int(idEstagio_final_custo_superior - a_estagio_inicial) + 1, std::vector<double>()));
		SmartEnupla<IdEstagio, std::vector<double>> custo_superior_problema_mestre(a_estagio_inicial, std::vector<std::vector<double>>(int(idEstagio_final_custo_superior - a_estagio_inicial) + 1, std::vector<double>()));
		...
		if (!a_simulacao && ((a_idIteracao > IdIteracao(iteracao_inicial + 1)) || ((a_idIteracao > iteracao_inicial) && iteracao_inicial > IdIteracao_0)))
		...
		for (IdEstagio idEstagio = a_estagio_inicial; idEstagio <= a_estagio_final; idEstagio++) {
			...
			if ((!a_simulacao) && (idEstagio == a_estagio_final))
				break;
			...
			if (numero_cenarios > 0) {
				...
				a_modeloOtimizacao.ativarCortesExternos(idEstagio);
		...
		executarPDDE_atualizarCustoSuperior_FW(a_idIteracao, a_estagio_final, custo_superior, a_modeloOtimizacao);
		...
		if (getSizeVetor(AttVetorMetodoSolucao_tempo_medio_solver_fw) > 0) {
			const IdIteracao idIter_eval = IdIteracao(getIteradorFinal(AttVetorMetodoSolucao_tempo_medio_solver_fw, IdIteracao()) + 1);
			if (a_idIteracao > idIter_eval) {
				for (IdIteracao idIter = idIter_eval; idIter < a_idIteracao; idIter++)
					addElemento(AttVetorMetodoSolucao_tempo_medio_solver_fw, idIter, 0.0);
			}
		}
		...
		if (a_simulacao) {
			const IdIteracao iteracao_final = a_modeloOtimizacao.arranjoResolucao.getAtributo(AttComumArranjoResolucao_iteracao_final, IdIteracao());
			const IdIteracao iteracao_numero_maximo = a_modeloOtimizacao.arranjoResolucao.getAtributo(AttComumArranjoResolucao_iteracao_numero_maximo, IdIteracao());
			if ((a_idIteracao == iteracao_final) && (iteracao_numero_maximo < iteracao_final)) {
...
void MetodoSolucao::executarPDDE_backward_new(EntradaSaidaDados a_entradaSaidaDados, const IdEstagio a_estagio_inicial, const IdEstagio a_estagio_final, const IdIteracao a_idIteracao, const IdProcesso a_idProcesso, const IdProcesso a_maiorIdProcesso, ModeloOtimizacao& a_modeloOtimizacao, Dados& a_dados) {
	...
	try {
		...
		SmartEnupla<IdEstagio, std::vector<double>> custo_superior(a_estagio_final, std::vector<std::vector<double>>(1, std::vector<double>()));
		...
		for (IdEstagio idEstagio = a_estagio_final; idEstagio >= estagio_inicial; idEstagio--) {
			...
			for (int c = 0; c < int(cenarios_estados.size()); c++) {
				...
				if (c == 0)
					a_modeloOtimizacao.anularCortesExternos(idEstagio);
				...
				for (IdAbertura idAbertura = menor_abertura_em_cenario_estado; idAbertura <= maior_abertura_em_cenario_estado; idAbertura++) {
					...
					try {
						...
						if (idEstagio == a_estagio_final)
							custo_superior.at(idEstagio).push_back(custo_total_otimizacao);
			...
			a_entradaSaidaDados.setDiretorioSaida(diretorio_iteracao);
			a_modeloOtimizacao.removerCortesBendersExternos(idEstagio, a_idIteracao, a_idProcesso, a_entradaSaidaDados);
			a_modeloOtimizacao.removerVarEstadoExterna(idEstagio, a_idIteracao);
		...
		for (IdEstagio idEstagio = a_estagio_final; idEstagio >= estagio_inicial; idEstagio--) {
			...
			if (a_idProcesso == IdProcesso_mestre) {
				if (((a_idIteracao == a_modeloOtimizacao.arranjoResolucao.getAtributo(AttComumArranjoResolucao_iteracao_final, IdIteracao())) && (a_idIteracao == a_modeloOtimizacao.arranjoResolucao.getAtributo(AttComumArranjoResolucao_iteracao_numero_maximo, IdIteracao()))) || (a_modeloOtimizacao.getAtributo(AttComumModeloOtimizacao_imprimir_corte_por_iteracao, bool()))) {
					a_entradaSaidaDados.setAppendArquivo(false);
					if (a_modeloOtimizacao.getAtributo(AttComumModeloOtimizacao_imprimir_corte_por_iteracao, bool()))
						a_entradaSaidaDados.setDiretorioSaida(diretorio_iteracao);
					else
						a_entradaSaidaDados.setDiretorioSaida(diretorio);
			...
			if ((a_idIteracao == a_modeloOtimizacao.arranjoResolucao.getAtributo(AttComumArranjoResolucao_iteracao_final, IdIteracao())) && (a_idIteracao == a_modeloOtimizacao.arranjoResolucao.getAtributo(AttComumArranjoResolucao_iteracao_numero_maximo, IdIteracao())))
				a_modeloOtimizacao.removerCorteBenders(idEstagio);

		} // for (IdEstagio idEstagio = a_estagio_final; idEstagio >= a_estagio_inicial; idEstagio--) {

		executarPDDE_atualizarCustoSuperior_BW(a_idIteracao, custo_superior, a_modeloOtimizacao);
...
void MetodoSolucao::executarPDDE_atualizarCustoSuperior_FW(const IdIteracao a_idIteracao, const IdEstagio a_idEstagioEnd_fw, const SmartEnupla<IdEstagio, std::vector<double>>& a_custo_superior, ModeloOtimizacao& a_modeloOtimizacao) {
		...
		if (idProcesso_local == IdProcesso_mestre) {
			...
			for (IdCenario idCenario = menor_cenario_iteracao; idCenario <= maior_cenario_iteracao; idCenario++) {
				...
				for (IdProcesso idProcesso_cenario_estado = IdProcesso_mestre; idProcesso_cenario_estado <= a_modeloOtimizacao.arranjoResolucao.getMaiorId(IdProcesso()); idProcesso_cenario_estado++) {
					if (a_modeloOtimizacao.arranjoResolucao.getSize1Matriz(a_idIteracao, idProcesso_cenario_estado, AttMatrizProcesso_cenario_estado_por_cenario) > 0) {
						if ((a_modeloOtimizacao.arranjoResolucao.getIterador1Inicial(a_idIteracao, idProcesso_cenario_estado, AttMatrizProcesso_cenario_estado_por_cenario, IdCenario()) <= idCenario) && (idCenario <= a_modeloOtimizacao.arranjoResolucao.getIterador1Final(a_idIteracao, idProcesso_cenario_estado, AttMatrizProcesso_cenario_estado_por_cenario, IdCenario()))) {
							idCenario_estado = a_modeloOtimizacao.arranjoResolucao.getElementoMatriz(a_idIteracao, idProcesso_cenario_estado, AttMatrizProcesso_cenario_estado_por_cenario, idCenario, a_idEstagioEnd_fw, IdCenario());
							break;
						}
					}
				}
				...
				if (idCenario_estado != IdCenario_Nenhum) {
					...
					for (IdEstagio idEstagio = a_idEstagioEnd_fw; idEstagio > menor_estagio; idEstagio--) {
			...
			for (IdEstagio idEstagio_prob = estagio_final; idEstagio_prob >= estagio_inicial; idEstagio_prob--) {
				...
				if (a_modeloOtimizacao.getSize1Matriz(IdProcessoEstocastico_1, AttMatrizProcessoEstocastico_probabilidade_realizacao) > 0) {

					const Periodo perIni = a_modeloOtimizacao.getIterador1Inicial(IdProcessoEstocastico_1, AttMatrizProcessoEstocastico_probabilidade_realizacao, Periodo());
					const Periodo perEnd = a_modeloOtimizacao.getIterador1Final(IdProcessoEstocastico_1, AttMatrizProcessoEstocastico_probabilidade_realizacao, Periodo());
					...
					if ((perIni <= period) && (period <= perEnd)) {
						const double prob_1 = a_modeloOtimizacao.getElementoMatriz(IdProcessoEstocastico_1, AttMatrizProcessoEstocastico_probabilidade_realizacao, period, IdRealizacao_1, double());
						const IdRealizacao idRealEnd = a_modeloOtimizacao.getIterador2Final(IdProcessoEstocastico_1, AttMatrizProcessoEstocastico_probabilidade_realizacao, period, IdRealizacao());
						bool is_equ = true;
						for (IdRealizacao idReal = IdRealizacao_2; idReal <= idRealEnd; idReal++) {
							if (prob_1 != a_modeloOtimizacao.getElementoMatriz(IdProcessoEstocastico_1, AttMatrizProcessoEstocastico_probabilidade_realizacao, period, idReal, double())) {
								is_equ = false;
								break;
							}
						}
						if (!is_equ) {
							for (IdCenario idCenario = menor_cenario_iteracao; idCenario <= maior_cenario_iteracao; idCenario++) {
								const IdRealizacao idReal = a_modeloOtimizacao.getElementoMatriz(IdProcessoEstocastico_1, AttMatrizProcessoEstocastico_mapeamento_espaco_amostral, idCenario, period, IdRealizacao());
								prob_cen.at(idCenario) *= a_modeloOtimizacao.getElementoMatriz(IdProcessoEstocastico_1, AttMatrizProcessoEstocastico_probabilidade_realizacao, period, idReal, double());
							} // for (IdCenario idCenario = menor_cenario_iteracao; idCenario <= maior_cenario_iteracao; idCenario++) {
						}
					} // if ((perIni <= period) && (period <= perEnd)) {
				} // if (a_modeloOtimizacao.getSize1Matriz(IdProcessoEstocastico_1, AttMatrizProcessoEstocastico_probabilidade_realizacao) > 0) {
			...
			if (getSizeVetor(AttVetorMetodoSolucao_custo_superior) > 0) {
				...
				if (a_idIteracao > idIter_eval) {
					for (IdIteracao idIter = idIter_eval; idIter < a_idIteracao; idIter++)
						addElemento(AttVetorMetodoSolucao_custo_superior, idIter, 0.0);
				}
			}
	...
	catch (const std::exception& erro) { throw std::invalid_argument("MetodoSolucao(" + getString(getIdObjeto()) + ")::executarPDDE_atualizarCustoSuperior_FW(" + getFullString(a_idIteracao) + ",a_custo_superior,a_probabilidade_cenario,a_modeloOtimizacao): \n" + std::string(erro.what())); }
...
void MetodoSolucao::executarPDDE_atualizarCustoSuperior_BW(const IdIteracao a_idIteracao, const SmartEnupla<IdEstagio, std::vector<double>>& a_custo_superior, ModeloOtimizacao& a_modeloOtimizacao) {
	...
	try {
		...
		if (idProcesso_local == IdProcesso_mestre) {
			...
			const IdEstagio idEstagio = menor_estagio;
			...
			if (a_custo_superior.size() > 0) {
				if (a_custo_superior.at(idEstagio).size() > 0) {
					...
					for (int i = 0; i < int(cenarios_estados.size()); i++) {
						...
						const IdCenario cenario_estado = cenarios_estados.at(i);
						const IdAbertura menor_abertura_em_cenario_estado = a_modeloOtimizacao.arranjoResolucao.getElementoMatriz(a_idIteracao, idProcesso_local, AttMatrizProcesso_menor_abertura_por_cenario_estado, cenario_estado, idEstagio, IdAbertura());
						const IdAbertura maior_abertura_em_cenario_estado = a_modeloOtimizacao.arranjoResolucao.getElementoMatriz(a_idIteracao, idProcesso_local, AttMatrizProcesso_maior_abertura_por_cenario_estado, cenario_estado, idEstagio, IdAbertura());
					...
					if (c != int(a_custo_superior.at(idEstagio).size()))
						throw std::invalid_argument("Erro em " + getFullString(idProcesso_local) + " " + getString(c) + " " + getString(int(a_custo_superior.at(idEstagio).size())));
			...
			for (IdProcesso idProcesso = IdProcesso_1; idProcesso <= a_modeloOtimizacao.arranjoResolucao.getMaiorId(IdProcesso()); idProcesso++) {
				...
				int numero_cenarios_outro = 0;
				MPI_Recv(&numero_cenarios_outro, 1, MPI_INT, getRank(idProcesso), 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

				if (numero_cenarios_outro > 0) {
					std::vector<double> custo_superior_outro(numero_cenarios_outro, NAN);
					...
					for (int i = 0; i < int(cenarios_estados.size()); i++) {
						...
						const IdCenario cenario_estado = cenarios_estados.at(i);
						const IdAbertura menor_abertura_em_cenario_estado = a_modeloOtimizacao.arranjoResolucao.getElementoMatriz(a_idIteracao, idProcesso, AttMatrizProcesso_menor_abertura_por_cenario_estado, cenario_estado, idEstagio, IdAbertura());
						const IdAbertura maior_abertura_em_cenario_estado = a_modeloOtimizacao.arranjoResolucao.getElementoMatriz(a_idIteracao, idProcesso, AttMatrizProcesso_maior_abertura_por_cenario_estado, cenario_estado, idEstagio, IdAbertura());
						const int na = int(maior_abertura_em_cenario_estado - menor_abertura_em_cenario_estado) + 1;
					...
					if (c != numero_cenarios_outro)
						throw std::invalid_argument("Erro em " + getFullString(idProcesso) + " " + getString(c) + " " + getString(numero_cenarios_outro));
			...
			for (IdEstagio idEstagio = menor_estagio; idEstagio <= maior_estagio; idEstagio++) {
				...
				MPI_Send(&numero_cenarios, 1, MPI_INT, getRank(IdProcesso_mestre), 1, MPI_COMM_WORLD);

				if (numero_cenarios > 0)
					MPI_Send(&a_custo_superior.at(idEstagio)[0], numero_cenarios, MPI_DOUBLE, getRank(IdProcesso_mestre), 2, MPI_COMM_WORLD);
	...
	catch (const std::exception& erro) { throw std::invalid_argument("MetodoSolucao(" + getString(getIdObjeto()) + ")::executarPDDE_atualizarCustoSuperior_BW(" + getFullString(a_idIteracao) + ",a_custo_superior,a_probabilidade_cenario,a_modeloOtimizacao): \n" + std::string(erro.what())); }
...
void MetodoSolucao::executarPDDE_atualizarCustoInferior(const IdIteracao a_idIteracao, const SmartEnupla<IdEstagio, std::vector<double>>& a_custo_inferior, ModeloOtimizacao& a_modeloOtimizacao){
	...
	try{
		...
		if (idProcesso_local == IdProcesso_mestre) {
			...
			if (getSizeVetor(AttVetorMetodoSolucao_custo_inferior) > 0) {
				const IdIteracao idIter_eval = IdIteracao(getIteradorFinal(AttVetorMetodoSolucao_custo_inferior, IdIteracao()) + 1);
				if (a_idIteracao > idIter_eval) {
					for (IdIteracao idIter = idIter_eval; idIter < a_idIteracao; idIter++)
						addElemento(AttVetorMetodoSolucao_custo_inferior, idIter, 0.0);
				}
			}
...
bool MetodoSolucao::executarPDDE_avaliarSolucao(EntradaSaidaDados a_entradaSaidaDados, const IdIteracao a_idIteracao, const IdProcesso a_idProcesso, ModeloOtimizacao &a_modeloOtimizacao){

	try {

		int encerrar_iteracao = 0;
		...
		if (a_idProcesso == IdProcesso_mestre) {
			...
			if (getSizeVetor(AttVetorMetodoSolucao_tempo_execucao_acumulado) > 0) {
				const IdIteracao idIter_eval = IdIteracao(getIteradorFinal(AttVetorMetodoSolucao_tempo_execucao_acumulado, IdIteracao()) + 1);
				if (a_idIteracao > idIter_eval) {
					for (IdIteracao idIter = idIter_eval; idIter < a_idIteracao; idIter++)
						addElemento(AttVetorMetodoSolucao_tempo_execucao_acumulado, idIter, 0.0);
				}
			}
			...
			const double custo_superior = getElementoVetor(AttVetorMetodoSolucao_custo_superior, a_idIteracao, double());
		...
		MPI_Bcast(&encerrar_iteracao, 1, MPI_INT, getRank(IdProcesso_mestre), MPI_COMM_WORLD);

		if (encerrar_iteracao == 1)
			return true;
