...
void LeituraCEPEL::leitura_CURVA_202001_NW27(Dados& a_dados, std::string a_nomeArquivo)
{
	...
	try {
		...
		if (myfile.is_open()) {
			...
			SmartEnupla<IdHidreletrica, SmartEnupla<Periodo, double>> lista_VOLMINOP(a_dados.getMenorId(IdHidreletrica()), std::vector<SmartEnupla<Periodo, double>>(int(a_dados.getMaiorId(IdHidreletrica()) - a_dados.getMenorId(IdHidreletrica())) + 1, SmartEnupla<Periodo, double>()));
...
void LeituraCEPEL::leitura_DGER_201908_NW25(Dados &a_dados, std::string nomeArquivo) {
	...
	try {
		...
		if (leituraArquivo.is_open()) {
			...
			if ((!dadosPreConfig_instanciados) || (a_dados.getAtributo(AttComumDados_numero_aberturas, int()) == 0))
			...
			/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			//Determina o vetor horizonte_otimização 
			/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			//Determina o vetor horizonte_estudo
			/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if (a_dados.getSizeVetor(AttVetorDados_horizonte_estudo) == 0) {
				...
				if (a_dados.getSizeMatriz(AttMatrizDados_horizonte_estudo) == 0) {
					...
					SmartEnupla<Periodo, IdEstagio> horizonte_estudo(a_dados.getMatriz(AttMatrizDados_horizonte_estudo, Periodo(), IdPatamarCarga(), double()), IdEstagio_Nenhum);
					...
					for (Periodo periodo = horizonte_estudo.getIteradorInicial(); periodo <= horizonte_estudo.getIteradorFinal(); horizonte_estudo.incrementarIterador(periodo)) {
						...
						for (IdEstagio idEstagio = IdEstagio_1; idEstagio <= idEstagio_final_otimizacao; idEstagio++) {
							...
							if (sobreposicao == 1.0) {
								horizonte_estudo.at(periodo) = idEstagio;
								break;
							}
							else if (sobreposicao > 0.0)
								throw std::invalid_argument("O periodo de estudo " + getString(periodo) + " nao deve incidir parcialmente sobre o periodo de otimizacao " + getString(periodo_otimizacao) + ".");
			...
			while (std::getline(leituraArquivo, line)) {
				...
				if (hidreletricasPreConfig_instanciadas) {
					...
					//Instancia as usinas com expansão (usinas não existentes na atualidade)
						...
						lista_hidreletrica_maiorIdConjuntoHidraulico.setElemento(idHidreletrica_expansao, IdConjuntoHidraulico_Nenhum);
						lista_hidreletrica_com_expansao.setElemento(idHidreletrica_expansao, usina_com_expansao);
						lista_hidreletrica_com_modificacao.setElemento(idHidreletrica_expansao, usina_com_modificacao);
						lista_hidreletrica_sem_capacidade.setElemento(idHidreletrica_expansao, usina_sem_capacidade);
			...
			while (std::getline(leituraArquivo, line)) {
				...
				if ((considerar_usina) ) {
					...
					if (idTermeletrica_inicializado == IdTermeletrica_Nenhum){					
						...
						lista_classe_ONS_termeletrica.setElemento(idTermeletrica, classe_CEPEL);
						lista_termeletrica_com_expansao.setElemento(idTermeletrica, usina_com_expansao);
...
void LeituraCEPEL::leitura_CLAST_201908_NW25(Dados& a_dados, std::string nomeArquivo) {
	...
	try {
		...
		SmartEnupla<IdTermeletrica, SmartEnupla<Periodo, double>> lista_custo_operacao(menorIdTermeletrica, std::vector<SmartEnupla<Periodo, double>>(int(maiorIdTermeletrica - menorIdTermeletrica) + 1, SmartEnupla<Periodo, double>(horizonte_estudo_DECK, 0.0)));
		...
		if (leituraArquivo.is_open()) {
			...
			for (IdTermeletrica idTermeletrica = lista_custo_operacao.getIteradorInicial(); idTermeletrica <= lista_custo_operacao.getIteradorFinal(); a_dados.vetorTermeletrica.incr(idTermeletrica)) {
			...
			} // for (IdTermeletrica idTermeletrica = a_dados.getMenorId(IdTermeletrica()); idTermeletrica <= lista_custo_operacao.getIteradorFinal(); a_dados.vetorTermeletrica.incr(idTermeletrica)) {
		...
		if (leituraArquivo.is_open()) {
			...
			if (duracao_patamar_carga_variavel_por_ano == true) {
				...
				if(!is_carregado_percentual_duracao_patamar_carga)
					a_dados.setMatriz(AttMatrizDados_horizonte_estudo, matriz_periodo_patamar_zero);
						...
						if (!is_carregado_percentual_duracao_patamar_carga) {
							...
							for (Periodo periodo = horizonte_estudo.getIteradorInicial(); periodo <= horizonte_estudo.getIteradorFinal(); horizonte_estudo.incrementarIterador(periodo)) {
								...
								if (sobreposicao > 0.0) {
									...
									const double valor_antigo = a_dados.getElementoMatriz(AttMatrizDados_horizonte_estudo, periodo, idPatamarCarga, double());

									a_dados.setElemento(AttMatrizDados_horizonte_estudo, periodo, idPatamarCarga, valor_antigo + sobreposicao * percentual_duracao_patamar_carga);
				...
				if (!is_carregado_percentual_duracao_patamar_carga) {
					...
					for (Periodo periodo = periodo_inicial; periodo <= periodo_final; horizonte_estudo.incrementarIterador(periodo)) {
						...
						for (int patamar = 0; patamar < numero_patamares; patamar++) {
							...
							a_dados.setElemento(AttMatrizDados_horizonte_estudo, periodo, getIdPatamarCargaFromChar(std::to_string(patamar + 1).c_str()), percentual_duracao_patamar_carga);
				...
				for (Periodo periodo = horizonte_estudo.getIteradorInicial(); periodo <= horizonte_estudo.getIteradorFinal(); horizonte_estudo.incrementarIterador(periodo)) {
					...
					if (periodo < periodo_pos_manutencao) {
						...
						if (((periodo >= periodo_manutencao_inicial) && (periodo <= periodo_manutencao_final)) || (sobreposicao > 0.0)) {
							...
							for (IdPatamarCarga idPatamarCarga = IdPatamarCarga_1; idPatamarCarga <= a_dados.getIterador2Final(AttMatrizDados_horizonte_estudo, periodo, IdPatamarCarga()); idPatamarCarga++)
								potencia_maxima += a_dados.getElementoMatriz(idTermeletrica, IdUnidadeUTE_1, AttMatrizUnidadeUTE_potencia_maxima, periodo, idPatamarCarga, double()) * a_dados.getElementoMatriz(AttMatrizDados_horizonte_estudo, periodo, idPatamarCarga, double());
...
void LeituraCEPEL::leitura_DSVAGUA_201908_NW25(Dados& a_dados, std::string nomeArquivo) {
	...
	try {
		...
		if (leituraArquivo.is_open()) {
			...
			SmartEnupla<IdHidreletrica, SmartEnupla<Periodo, double>> lista_DESVAGUA(a_dados.getMenorId(IdHidreletrica()), std::vector<SmartEnupla<Periodo, double>>(int(a_dados.getMaiorId(IdHidreletrica()) - a_dados.getMenorId(IdHidreletrica())) + 1, SmartEnupla<Periodo, double>()));
			...
			for (IdHidreletrica idHidreletrica = a_dados.getMenorId(IdHidreletrica()); idHidreletrica <= lista_DESVAGUA.getIteradorFinal(); a_dados.vetorHidreletrica.incr(idHidreletrica)) {
			...
			if (leituraArquivo.is_open()) {
				...
				while (true) {
					...
					if (idAgrupamentoIntercambio_inicializado == IdAgrupamentoIntercambio_Nenhum) { // Novo AgrupamentoIntercambio
						...
						lista_codigo_ONS_agrupamento_intercambio.setElemento(idAgrupamentoIntercambio, codigo_usina);
				...
				while (true) {
					...
					/////////////////////////////////////////////////
						...
						if (line_size > 29 + 8 * int(maiorIdPatamarCarga_deck - 1)) {//29 + 8 * int(maiorIdPatamarCarga_global - 1) refere-se ao tamanho da linha até o último campo obrigatório
							...
							atributo = line.substr(29 + 8 * int(maiorIdPatamarCarga_deck - 1), line_size - (29 + 8 * int(a_dados.getIterador2Final(AttMatrizDados_horizonte_estudo, horizonte_estudo.getIteradorInicial(), IdPatamarCarga()) - 1)) + 1);
							atributo.erase(std::remove(atributo.begin(), atributo.end(), ' '), atributo.end());
...
void LeituraCEPEL::leitura_GHMIN_201908_NW25(Dados& a_dados, std::string nomeArquivo) {
	...
	try {
		...
		SmartEnupla<IdHidreletrica, SmartEnupla<Periodo, SmartEnupla<IdPatamarCarga, double>>> lista_GHMIN(a_dados.getMenorId(IdHidreletrica()), std::vector<SmartEnupla<Periodo, SmartEnupla<IdPatamarCarga, double>>>(int(a_dados.getMaiorId(IdHidreletrica()) - a_dados.getMenorId(IdHidreletrica())) + 1, SmartEnupla<Periodo, SmartEnupla<IdPatamarCarga, double>>()));
		...
		if (leituraArquivo.is_open()) {
			...
			while (true) {
				...
				if (ano != IdAno_Nenhum) {//Ignora valores para periodos estáticos PRE/POS
					...
					if (idPatamarCarga_lido == IdPatamarCarga_Nenhum)
						potencia_minima_patamar = SmartEnupla<IdPatamarCarga, double>(IdPatamarCarga_1, std::vector<double>(a_dados.getIterador2Final(AttMatrizDados_horizonte_estudo, horizonte_estudo.getIteradorInicial(), IdPatamarCarga()), potencia_minima));

					else {
						potencia_minima_patamar = SmartEnupla<IdPatamarCarga, double>(IdPatamarCarga_1, std::vector<double>(a_dados.getIterador2Final(AttMatrizDados_horizonte_estudo, horizonte_estudo.getIteradorInicial(), IdPatamarCarga()), 0.0));
			...
			for (IdHidreletrica idHidreletrica = a_dados.getMenorId(IdHidreletrica()); idHidreletrica <= lista_GHMIN.getIteradorFinal(); a_dados.vetorHidreletrica.incr(idHidreletrica)) {
			...
			if (leituraArquivo.is_open()) {
				...
				while (true) {
					...
					if (true) {
						...
						lista_codigo_ONS_restricao_eletrica.setElemento(idRestricaoEletrica, codigo_usina_restricao);
...
void LeituraCEPEL::validacoes_NW(Dados & a_dados, const std::string a_diretorio) {
	...
	try {
		...
		if (idProcesso == IdProcesso_mestre && nomeArquivo_cortes_NW != "nenhum" && hidreletricasPreConfig_instanciadas) {
			...
			leitura_cortes_NEWAVE(a_dados, horizonte_estudo, nomeArquivo_cortes_NW, must_read_nwlistcf, diretorio_att_premissas, maior_ONS_REE, horizonte_processo_estocastico, a_dados.getMatriz(AttMatrizDados_horizonte_estudo, Periodo(), IdPatamarCarga(), double()));
