...
void LeituraCEPEL::leitura_cortes_NEWAVE(Dados& a_dados, const SmartEnupla<Periodo, IdEstagio> a_horizonte_estudo, std::string a_diretorio, std::string a_diretorio_cortes, std::string a_nomeArquivo)
{
	try {
		...
		if (leituraArquivo.is_open()) {
			...
			calcular_equacionamento_afluencia_natural_x_hidreletrica(a_dados, horizonte_tendencia_mais_estudo);
			calcular_equacionamento_afluencia_natural_x_hidreletrica_out_estudo(a_dados, horizonte_tendencia_mais_estudo);

			atualizar_codigo_usina_jusante_EAR(a_dados);//Tira o caminho de P.Afonso 123 e deixa só Comp-Mox como JUSENA de Xingó (necessário para o cálculo da natural)
			atualizar_equacionamento_afluencia_natural_x_hidreletrica(a_dados, horizonte_tendencia_mais_estudo); //Precisa-se atualizar as afluências naturais com o registro JUSENA (muda a jusante das usinas para cálculo de ENA)
			...
			if (imprimir_info_calculo_ENA) {//Somente para teste
				...
				imprime_produtibilidade_ENA(a_dados, a_diretorio + a_diretorio_cortes + "//_info_HIDRELETRICA_AttVetor_produtibilidade_ENA.csv");
...
void LeituraCEPEL::atualizar_codigo_usina_jusante_EAR(Dados& a_dados)
{
	try {

		//Tira a conexão JUSENA  de uma usina instanciada se a conexão JUSENA está em lista_hidreletrica_out_estudo (necesário para o cálculo de Afluência natural utilizando "caminho" JUSENA
		//Ex P.Afonso 123 é substituido por Comp-Mox-PAfonso
		...
		for (int pos = 1; pos <= int(lista_hidreletrica_out_estudo.size()); pos++) {
			...
			const int codigo_usina_jusante_EAR = lista_hidreletrica_out_estudo.at(pos).getAtributo(AttComumHidreletrica_codigo_usina_jusante_EAR, int());

			//Identifica a usina que aponta no registro JUSENA para a usina fora do estudo CP
			IdHidreletrica  idHidreletrica_montante = IdHidreletrica_Nenhum;

			for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {

				if (a_dados.vetorHidreletrica.att(idHidreletrica).getAtributo(AttComumHidreletrica_codigo_usina_jusante_EAR, int()) == codigo_usina_jusante_EAR) {
					a_dados.vetorHidreletrica.att(idHidreletrica).setAtributo(AttComumHidreletrica_codigo_usina_jusante_EAR, 0);

				}//if (a_dados.vetorHidreletrica.att(idHidreletrica).getAtributo(AttComumHidreletrica_codigo_usina_jusante_EAR, int()) == codigo_usina) {
			}//for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {

		}//for (int pos = 1; pos <= int(lista_hidreletrica_out_estudo.size()); pos++) {

	}//	try {
	catch (const std::exception& erro) { throw std::invalid_argument("LeituraCEPEL::atualizar_codigo_usina_jusante_EAR: \n" + std::string(erro.what())); }
...
double LeituraCEPEL::get_cota_para_conversao_cortes_NEWAVE(Hidreletrica& a_hidreletrica, const Periodo a_periodo, const Periodo a_periodo_inicial_horizonte_estudo, const double a_percentual_volume_util, const bool a_is_calculo_para_ENA)
{
	try {
		...
		if(a_hidreletrica.getSizeVetor(AttVetorHidreletrica_canal_fuga_medio) > 0 and a_periodo >= a_periodo_inicial_horizonte_estudo)//Para quem teve registro JUSENA e o período está dentro do horizonte de estudo pega o valor do canal_fuga_medio do AttVetor
...
void LeituraCEPEL::instancia_lista_hidreletrica_out_estudo_from_codigo_usina_jusante_EAR(Dados& a_dados, std::string a_nomeArquivo)
{
	try {
		...
		for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {
			...
			const int codigo_usina_jusante_EAR = a_dados.getAtributo(idHidreletrica, AttComumHidreletrica_codigo_usina_jusante_EAR, int());
			...
			if (idHidreletrica_jusante_EAR == IdHidreletrica_Nenhum && codigo_usina_jusante_EAR > 0) {//Significa que a usina a jusante_EAR não existe no CP (p.ex. COMP PAF-MOX)
			...
			}//if if (idHidreletrica_jusante_EAR == IdHidreletrica_Nenhum && codigo_usina_jusante_EAR > 0) {
						...
						while (int(line.length()) > 0) {
							...
							if (registro == registro_nomeUsina) {
								...
								if (idHidreletricaJusante != IdHidreletrica_Nenhum) { 
									...
									a_hidreletrica_out.setAtributo(AttComumHidreletrica_codigo_usina_jusante_EAR, codigo_jusante);
...
void LeituraCEPEL::calcular_equacionamento_afluencia_natural_x_REE(Dados& a_dados, const SmartEnupla<Periodo, bool> a_horizonte_tendencia_mais_estudo)
{
	try {
		...
		for (IdReservatorioEquivalente idReservatorioEquivalente = IdReservatorioEquivalente_1; idReservatorioEquivalente <= IdReservatorioEquivalente(maior_ONS_REE); idReservatorioEquivalente++) {
			...
			lista_coeficiente_idHidreletricas_calculo_ENA_x_REE_x_cenario_x_periodo.addElemento(idReservatorioEquivalente, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<IdHidreletrica, double>>>(IdCenario_1, std::vector<SmartEnupla<Periodo, SmartEnupla<IdHidreletrica, double>>>(idCenario_final, SmartEnupla<Periodo, SmartEnupla<IdHidreletrica, double>>(a_horizonte_tendencia_mais_estudo, SmartEnupla<IdHidreletrica, double>(menorIdHidreletrica, std::vector<double>(int(maiorIdHidreletrica - menorIdHidreletrica) + 1, 0.0))))));
			lista_termo_independente_calculo_ENA_x_REE_x_cenario_x_periodo.addElemento(idReservatorioEquivalente, SmartEnupla<IdCenario, SmartEnupla<Periodo, double>>(IdCenario_1, std::vector<SmartEnupla<Periodo, double>>(idCenario_final, SmartEnupla<Periodo, double>(a_horizonte_tendencia_mais_estudo, 0.0))));
			...
			if (retorna_is_idHidreletrica_in_calculo_ENA(a_dados.vetorHidreletrica.att(idHidreletrica).getAtributo(AttComumHidreletrica_codigo_usina, int()))) {
				...
				for (IdCenario idCenario = idCenario_inicial; idCenario <= idCenario_final; idCenario++) {
					...
					for (Periodo periodo = periodo_inicial; periodo <= periodo_final; a_horizonte_tendencia_mais_estudo.incrementarIterador(periodo)) {
						...
						SmartEnupla<int, double>			coeficiente_idHidreletricas_calculo_ENA = lista_coeficiente_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica).at(idCenario).getElemento(periodo);
						double								termo_independente_calculo_ENA = lista_termo_independente_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica).at(idCenario).getElemento(periodo);
						///////////////////////////////////////////////////////////
						...
						for (int pos = 1; pos <= int(idHidreletricas_calculo_ENA.size()); pos++) {
							...
							const IdHidreletrica idHidreletrica = idHidreletricas_calculo_ENA.getElemento(pos);

							const double coeficiente_idHidreletricas_anterior = lista_coeficiente_idHidreletricas_calculo_ENA_x_REE_x_cenario_x_periodo.at(idReservatorioEquivalente).at(idCenario).at(periodo).getElemento(idHidreletrica);
							...
							lista_coeficiente_idHidreletricas_calculo_ENA_x_REE_x_cenario_x_periodo.at(idReservatorioEquivalente).at(idCenario).at(periodo).setElemento(idHidreletrica, coeficiente_idHidreletricas_novo);

						}//for (int pos = 1; pos <= int(idHidreletricas_calculo_ENA.size()); pos++) {

						const double termo_independente_anterior = lista_termo_independente_calculo_ENA_x_REE_x_cenario_x_periodo.at(idReservatorioEquivalente).at(idCenario).getElemento(periodo);
						const double termo_independente_novo = termo_independente_anterior + produtibilidade_ENA * termo_independente_calculo_ENA;

						lista_termo_independente_calculo_ENA_x_REE_x_cenario_x_periodo.at(idReservatorioEquivalente).at(idCenario).setElemento(periodo, termo_independente_novo);
			...
			if (retorna_is_idHidreletrica_in_calculo_ENA(codigo_usina)) {
				...
				for (IdCenario idCenario = idCenario_inicial; idCenario <= idCenario_final; idCenario++) {
					...
					for (Periodo periodo = periodo_inicial; periodo <= periodo_final; a_horizonte_tendencia_mais_estudo.incrementarIterador(periodo)) {
						...
						///////////////////////////////////////////////////////////
						//Estrutura com a informação da componente das afluências naturais
						SmartEnupla<int, IdHidreletrica>	idHidreletricas_calculo_ENA = lista_idHidreletricas_calculo_ENA_x_codigo_usina_x_cenario_x_periodo.at(codigo_usina).at(idCenario).getElemento(periodo);
						SmartEnupla<int, double>			coeficiente_idHidreletricas_calculo_ENA = lista_coeficiente_idHidreletricas_calculo_ENA_x_codigo_usina_x_cenario_x_periodo.at(codigo_usina).at(idCenario).getElemento(periodo);
						double								termo_independente_calculo_ENA = lista_termo_independente_calculo_ENA_x_codigo_usina_x_cenario_x_periodo.at(codigo_usina).at(idCenario).getElemento(periodo);
						///////////////////////////////////////////////////////////
...
void LeituraCEPEL::calcular_equacionamento_afluencia_natural_x_hidreletrica(Dados& a_dados, const SmartEnupla<Periodo, bool> a_horizonte_tendencia_mais_estudo)
{
	try {
		...
		//SmartEnupla<IdHidreletrica, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>>(menorIdHidreletrica, std::vector<SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>>(int(maiorIdHidreletrica - menorIdHidreletrica) + 1, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>()));
		
		//lista_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo = SmartEnupla<IdHidreletrica, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>>(menorIdHidreletrica, std::vector<SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>>(int(maiorIdHidreletrica - menorIdHidreletrica) + 1, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>(IdCenario_1, std::vector<SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>(idCenario_final, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>(a_horizonte_tendencia_mais_estudo, SmartEnupla<int, IdHidreletrica>())))));
		//lista_coeficiente_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo = SmartEnupla<IdHidreletrica, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, double>>>>(menorIdHidreletrica, std::vector<SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, double>>>>(int(maiorIdHidreletrica - menorIdHidreletrica) + 1, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, double>>>(IdCenario_1, std::vector<SmartEnupla<Periodo, SmartEnupla<int, double>>>(idCenario_final, SmartEnupla<Periodo, SmartEnupla<int, double>>(a_horizonte_tendencia_mais_estudo, SmartEnupla<int, double>())))));
		//lista_termo_independente_calculo_ENA_x_hidreletrica_x_cenario_x_periodo = SmartEnupla<IdHidreletrica, SmartEnupla<IdCenario, SmartEnupla<Periodo, double>>>(menorIdHidreletrica, std::vector<SmartEnupla<IdCenario, SmartEnupla<Periodo, double>>>(int(maiorIdHidreletrica - menorIdHidreletrica) + 1, SmartEnupla<IdCenario, SmartEnupla<Periodo, double>>(IdCenario_1, std::vector<SmartEnupla<Periodo, double>>(idCenario_final, SmartEnupla<Periodo, double>(a_horizonte_tendencia_mais_estudo, 0.0)))));


		lista_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo = SmartEnupla<IdHidreletrica, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>>(menorIdHidreletrica, std::vector<SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>>(int(maiorIdHidreletrica - menorIdHidreletrica) + 1, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>()));
		lista_coeficiente_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo = SmartEnupla<IdHidreletrica, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, double>>>>(menorIdHidreletrica, std::vector<SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, double>>>>(int(maiorIdHidreletrica - menorIdHidreletrica) + 1, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, double>>>()));
		lista_termo_independente_calculo_ENA_x_hidreletrica_x_cenario_x_periodo = SmartEnupla<IdHidreletrica, SmartEnupla<IdCenario, SmartEnupla<Periodo, double>>>(menorIdHidreletrica, std::vector<SmartEnupla<IdCenario, SmartEnupla<Periodo, double>>>(int(maiorIdHidreletrica - menorIdHidreletrica) + 1, SmartEnupla<IdCenario, SmartEnupla<Periodo, double>>()));

		for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {

			lista_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.setElemento(idHidreletrica, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>(IdCenario_1, std::vector<SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>(idCenario_final, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>(a_horizonte_tendencia_mais_estudo, SmartEnupla<int, IdHidreletrica>()))));
			lista_coeficiente_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.setElemento(idHidreletrica, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, double>>>(IdCenario_1, std::vector<SmartEnupla<Periodo, SmartEnupla<int, double>>>(idCenario_final, SmartEnupla<Periodo, SmartEnupla<int, double>>(a_horizonte_tendencia_mais_estudo, SmartEnupla<int, double>()))));
			lista_termo_independente_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.setElemento(idHidreletrica, SmartEnupla<IdCenario, SmartEnupla<Periodo, double>>(IdCenario_1, std::vector<SmartEnupla<Periodo, double>>(idCenario_final, SmartEnupla<Periodo, double>(a_horizonte_tendencia_mais_estudo, 0.0))));
		...
		for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {
			...
			for (IdCenario idCenario = idCenario_inicial; idCenario <= idCenario_final; idCenario++) {
				...
				for (Periodo periodo = periodo_inicial; periodo <= periodo_final; a_horizonte_tendencia_mais_estudo.incrementarIterador(periodo)) {
					...
					retorna_equacionamento_regras_afluencia_natural_x_idHidreletrica(a_dados, idHidreletrica, idCenario, periodo_inicial, periodo, idHidreletricas_calculo_ENA, coeficiente_idHidreletricas_calculo_ENA, termo_independente_calculo_ENA);

					//Atualiza listas
					lista_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica).at(idCenario).setElemento(periodo, idHidreletricas_calculo_ENA);
					lista_coeficiente_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica).at(idCenario).setElemento(periodo, coeficiente_idHidreletricas_calculo_ENA);
					lista_termo_independente_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica).at(idCenario).setElemento(periodo, termo_independente_calculo_ENA);
...
void LeituraCEPEL::calcular_equacionamento_afluencia_natural_x_hidreletrica_out_estudo(Dados& a_dados, const SmartEnupla<Periodo, bool> a_horizonte_tendencia_mais_estudo)
{
	try {
		...
		for (int pos = 1; pos <= int(lista_hidreletrica_out_estudo.size()); pos++) {
			...
			for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {
				...
				if (a_dados.vetorHidreletrica.att(idHidreletrica).getAtributo(AttComumHidreletrica_codigo_usina_jusante_EAR, int()) == codigo_usina) {
					idHidreletrica_montante = idHidreletrica;
					break;
				}//if (a_dados.vetorHidreletrica.att(idHidreletrica).getAtributo(AttComumHidreletrica_codigo_usina_jusante_EAR, int()) == codigo_usina) {
			}//for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {

			if(idHidreletrica_montante == IdHidreletrica_Nenhum){ throw std::invalid_argument("Nao encontrada idHidreletrica com codigo_usina_jusante_EAR: " + getString(codigo_usina) + "\n");}
			...
			lista_idHidreletricas_calculo_ENA_x_codigo_usina_x_cenario_x_periodo.addElemento(codigo_usina, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>(IdCenario_1, std::vector<SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>>(idCenario_final, SmartEnupla<Periodo, SmartEnupla<int, IdHidreletrica>>(a_horizonte_tendencia_mais_estudo, SmartEnupla<int, IdHidreletrica>()))));
			lista_coeficiente_idHidreletricas_calculo_ENA_x_codigo_usina_x_cenario_x_periodo.addElemento(codigo_usina, SmartEnupla<IdCenario, SmartEnupla<Periodo, SmartEnupla<int, double>>>(IdCenario_1, std::vector<SmartEnupla<Periodo, SmartEnupla<int, double>>>(idCenario_final, SmartEnupla<Periodo, SmartEnupla<int, double>>(a_horizonte_tendencia_mais_estudo, SmartEnupla<int, double>()))));
			lista_termo_independente_calculo_ENA_x_codigo_usina_x_cenario_x_periodo.addElemento(codigo_usina, SmartEnupla<IdCenario, SmartEnupla<Periodo, double>>(IdCenario_1, std::vector<SmartEnupla<Periodo, double>>(idCenario_final, SmartEnupla<Periodo, double>(a_horizonte_tendencia_mais_estudo, 0.0))));
			//////////////////////////////////////////////
			...
			for (IdCenario idCenario = idCenario_inicial; idCenario <= idCenario_final; idCenario++) {
				...
				for (Periodo periodo = periodo_inicial; periodo <= periodo_final; a_horizonte_tendencia_mais_estudo.incrementarIterador(periodo)) {
					...
					if (codigo_usina == 176) {//COMP-MOX
						...
						//Soma as incrementais de Moxotó, P.Afonso 123 e P.Afonso 4

						const int codigo_usina_Moxoto = 173;
						const int codigo_usina_P_Afonso_123 = 174;
						const int codigo_usina_P_Afonso_4 = 175;

						for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {

							if (a_dados.vetorHidreletrica.att(idHidreletrica).getAtributo(AttComumHidreletrica_codigo_usina, int()) == codigo_usina_Moxoto) {
								idHidreletricas_calculo_ENA.addElemento(int(idHidreletricas_calculo_ENA.size()) + 1, idHidreletrica);
								coeficiente_idHidreletricas_calculo_ENA.addElemento(int(coeficiente_idHidreletricas_calculo_ENA.size()) + 1, 1.0);
								break;
							}//if (a_dados.vetorHidreletrica.att(idHidreletrica).getAtributo(AttComumHidreletrica_codigo_usina, int()) == codigo_usina) {
						}//for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {

						for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {

							if (a_dados.vetorHidreletrica.att(idHidreletrica).getAtributo(AttComumHidreletrica_codigo_usina, int()) == codigo_usina_P_Afonso_123) {
								idHidreletricas_calculo_ENA.addElemento(int(idHidreletricas_calculo_ENA.size()) + 1, idHidreletrica);
								coeficiente_idHidreletricas_calculo_ENA.addElemento(int(coeficiente_idHidreletricas_calculo_ENA.size()) + 1, 1.0);
								break;
							}//if (a_dados.vetorHidreletrica.att(idHidreletrica).getAtributo(AttComumHidreletrica_codigo_usina, int()) == codigo_usina) {
						...
						for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {
							...
							if (a_dados.vetorHidreletrica.att(idHidreletrica).getAtributo(AttComumHidreletrica_codigo_usina, int()) == codigo_usina_P_Afonso_4) {
								idHidreletricas_calculo_ENA.addElemento(int(idHidreletricas_calculo_ENA.size()) + 1, idHidreletrica);
								coeficiente_idHidreletricas_calculo_ENA.addElemento(int(coeficiente_idHidreletricas_calculo_ENA.size()) + 1, 1.0);
								break;
							}//if (a_dados.vetorHidreletrica.att(idHidreletrica).getAtributo(AttComumHidreletrica_codigo_usina, int()) == codigo_usina) {
						...
						//Atualiza listas
						lista_idHidreletricas_calculo_ENA_x_codigo_usina_x_cenario_x_periodo.at(codigo_usina).at(idCenario).setElemento(periodo, idHidreletricas_calculo_ENA);
						lista_coeficiente_idHidreletricas_calculo_ENA_x_codigo_usina_x_cenario_x_periodo.at(codigo_usina).at(idCenario).setElemento(periodo, coeficiente_idHidreletricas_calculo_ENA);
						lista_termo_independente_calculo_ENA_x_codigo_usina_x_cenario_x_periodo.at(codigo_usina).at(idCenario).setElemento(periodo, termo_independente_calculo_ENA);
					...
					retorna_equacionamento_regras_afluencia_natural_x_idHidreletrica(a_dados, idHidreletrica_montante, idCenario, periodo_inicial, periodo, idHidreletricas_calculo_ENA, coeficiente_idHidreletricas_calculo_ENA, termo_independente_calculo_ENA);
...
void LeituraCEPEL::atualizar_equacionamento_afluencia_natural_x_hidreletrica(Dados& a_dados, const SmartEnupla<Periodo, bool> a_horizonte_tendencia_mais_estudo)
{
	try {
		...
		//////////////////////////////////////////////////////////////////////
		//Regra de negócio: (as afluências naturais devem ser atualizadas com a jusante de cálculo da ENA
		// 1. Cálcular as afluências incrementais utilizando a CONFIGURAÇÃO HIDRÁULICA do sistema das usinas a jusante (da configuração hidráulica e do novo "caminho" JUSENA) da usina com registro JUSENA
		// 2. Cálcular as novas afluências naturais seguindo as conexões JUSENA

		const IdHidreletrica menorIdHidreletrica = a_dados.getMenorId(IdHidreletrica());
		const IdHidreletrica maiorIdHidreletrica = a_dados.getMaiorId(IdHidreletrica());

		///////////////////////////////////////////////////////////////////////////////////////////////////////////
		//0. Identifica quais usinas precisam de uma atualização da afluência natural por causa do registro JUSENA
		///////////////////////////////////////////////////////////////////////////////////////////////////////////

		SmartEnupla<IdHidreletrica, bool> is_atualizar_afluencia_natural(menorIdHidreletrica, std::vector<bool> (int(maiorIdHidreletrica - menorIdHidreletrica) + 1, false));
		...
		for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {
			...
			if (is_registro_JUSENA) {//Significa que a usina tem um registro JUSENA
				...
				int codigo_usina_alvo = codigo_usina_jusante_EAR;

				while (codigo_usina_alvo > 0) {

					bool is_codigo_usina_alvo_encontrado = false;

					///////////////////////////////////////////////
					//Procura nas usinas instanciadas no estudo CP
					///////////////////////////////////////////////
					for (IdHidreletrica idHidreletrica_aux = menorIdHidreletrica; idHidreletrica_aux <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica_aux)) {
						...
						if (a_dados.vetorHidreletrica.att(idHidreletrica_aux).getAtributo(AttComumHidreletrica_codigo_usina, int()) == codigo_usina_alvo) {
							is_codigo_usina_alvo_encontrado = true;
							is_atualizar_afluencia_natural.setElemento(idHidreletrica_aux, true);

							//Atualiza codigo_usina_alvo
							codigo_usina_alvo = 0;

							const IdHidreletrica idHidreletrica_jusante = a_dados.vetorHidreletrica.att(idHidreletrica_aux).getAtributo(AttComumHidreletrica_jusante, IdHidreletrica());
					...
					///////////////////////////////////////////////
					//Procura nas usinas NÃO instanciadas no estudo CP
					// Atualiza o codigo_usina_alvo para atualizar_afluencia_natural
					///////////////////////////////////////////////

					if (!is_codigo_usina_alvo_encontrado) {
						...
						for (int pos = 1; pos <= int(lista_hidreletrica_out_estudo.size()); pos++) {
							...
							if (lista_hidreletrica_out_estudo.at(pos).getAtributo(AttComumHidreletrica_codigo_usina, int()) == codigo_usina_alvo) {
								...
								const IdHidreletrica idHidreletrica_jusante = lista_hidreletrica_out_estudo.at(pos).getAtributo(AttComumHidreletrica_jusante, IdHidreletrica());
				...
				////////////////////////////////////////////////////////////////////
				//1.2 Identifica as usinas a jusante do caminho "cortado" por JUSENA
				////////////////////////////////////////////////////////////////////
		...
		for (IdCenario idCenario = idCenario_inicial; idCenario <= idCenario_final; idCenario++) {
			...
			for (Periodo periodo = periodo_inicial; periodo <= periodo_final; a_horizonte_tendencia_mais_estudo.incrementarIterador(periodo)) {
				...
				SmartEnupla<IdHidreletrica, SmartEnupla<int, IdHidreletrica>>    aux_lista_idHidreletricas_calculo_ENA_x_hidreletrica(menorIdHidreletrica, std::vector<SmartEnupla<int, IdHidreletrica>>(int(maiorIdHidreletrica - menorIdHidreletrica) + 1, SmartEnupla<int, IdHidreletrica>()));
				SmartEnupla<IdHidreletrica, SmartEnupla<int, double>>			 aux_lista_coeficiente_idHidreletricas_calculo_ENA_x_hidreletrica(menorIdHidreletrica, std::vector<SmartEnupla<int, double>>(int(maiorIdHidreletrica - menorIdHidreletrica) + 1, SmartEnupla<int, double>()));
				SmartEnupla<IdHidreletrica, double>							     aux_lista_termo_independente_calculo_ENA_x_hidreletrica(menorIdHidreletrica, std::vector<double>(int(maiorIdHidreletrica - menorIdHidreletrica) + 1, 0.0));
						
				for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {

					aux_lista_idHidreletricas_calculo_ENA_x_hidreletrica.setElemento(idHidreletrica, lista_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica).at(idCenario).getElemento(periodo));
					aux_lista_coeficiente_idHidreletricas_calculo_ENA_x_hidreletrica.setElemento(idHidreletrica, lista_coeficiente_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica).at(idCenario).getElemento(periodo));
					aux_lista_termo_independente_calculo_ENA_x_hidreletrica.setElemento(idHidreletrica, lista_termo_independente_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica).at(idCenario).getElemento(periodo));
				...
				for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {
					...
					if (is_atualizar_afluencia_natural.getElemento(idHidreletrica)) {
						...
						for (IdHidreletrica idHidreletrica_montante = menorIdHidreletrica; idHidreletrica_montante <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica_montante)) {
							...
							if (a_dados.vetorHidreletrica.att(idHidreletrica_montante).getAtributo(AttComumHidreletrica_jusante, IdHidreletrica()) == idHidreletrica) {
								...
								for (int pos = 1; pos <= int(montante_idHidreletricas_calculo_ENA.size()); pos++) {
									idHidreletricas_calculo_ENA.addElemento(int(idHidreletricas_calculo_ENA.size()) + 1, montante_idHidreletricas_calculo_ENA.getElemento(pos));
									coeficiente_idHidreletricas_calculo_ENA.addElemento(int(coeficiente_idHidreletricas_calculo_ENA.size()) + 1, -montante_coeficiente_idHidreletricas_calculo_ENA.getElemento(pos));
								}//for (int pos = 1; pos <= int(montante_idHidreletricas_calculo_ENA.size()); pos++) {

								termo_independente_calculo_ENA -= montante_termo_independente_calculo_ENA;
		...
		while (true) { //Faz de forma dinâmica para garantir que o cálculo da afluência natural de uma usina seja com base na natural já calculada das usinas a montante

			bool is_all_afluencia_natural_atualizada = true;
			...
			for (IdHidreletrica idHidreletrica = menorIdHidreletrica; idHidreletrica <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica)) {
				...
				if (is_atualizar_afluencia_natural.getElemento(idHidreletrica)) {
					...
					is_all_afluencia_natural_atualizada = false;

					/////////////////////////////////////////////////////////////////////////////////////
					//Verifica que todas as usinas a montante já tem suas afluências naturais calculadas
					/////////////////////////////////////////////////////////////////////////////////////

					bool is_all_usinas_montantes_com_afluencia_natural_atualizada = true;

					//Premissa: as usinas com montante em lista_hidreletrica_out_estudo já consideram is_all_usinas_montantes_com_afluencia_natural_atualizada = true
					//porque são previamente atualizadas no método calcular_equacionamento_afluencia_natural_x_hidreletrica_out_estudo (caso COMP-MOX-PAFONSO)

					for (IdHidreletrica idHidreletrica_montante = menorIdHidreletrica; idHidreletrica_montante <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica_montante)) {

						if (a_dados.vetorHidreletrica.att(idHidreletrica_montante).getAtributo(AttComumHidreletrica_codigo_usina_jusante_EAR, int()) == codigo_usina) {

							if (is_atualizar_afluencia_natural.getElemento(idHidreletrica_montante)) {
								is_all_usinas_montantes_com_afluencia_natural_atualizada = false;
								break;
					...
					//****************************
					//Atualiza afluência natural
					//****************************

					if (is_all_usinas_montantes_com_afluencia_natural_atualizada) {
						...
						for (IdCenario idCenario = idCenario_inicial; idCenario <= idCenario_final; idCenario++) {
							...
							for (Periodo periodo = periodo_inicial; periodo <= periodo_final; a_horizonte_tendencia_mais_estudo.incrementarIterador(periodo)) {
								...
								//////////////////////////////////////////
								//Atualiza com as usinas dentro do estudo
								//////////////////////////////////////////
								...
								for (IdHidreletrica idHidreletrica_montante = menorIdHidreletrica; idHidreletrica_montante <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica_montante)) {
									...
									if (a_dados.vetorHidreletrica.att(idHidreletrica_montante).getAtributo(AttComumHidreletrica_codigo_usina_jusante_EAR, int()) == codigo_usina) {
										...
										//Afluência natural usina a montante
										const SmartEnupla<int, IdHidreletrica>	    montante_idHidreletricas_calculo_ENA = lista_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica_montante).at(idCenario).getElemento(periodo);
										const SmartEnupla<int, double>	montante_coeficiente_idHidreletricas_calculo_ENA = lista_coeficiente_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica_montante).at(idCenario).getElemento(periodo);
										const double								 montante_termo_independente_calculo_ENA = lista_termo_independente_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica_montante).at(idCenario).getElemento(periodo);

										//Cálculo afluência natural
										SmartEnupla<int, IdHidreletrica>	idHidreletricas_calculo_ENA = lista_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica).at(idCenario).getElemento(periodo);
										SmartEnupla<int, double>			coeficiente_idHidreletricas_calculo_ENA = lista_coeficiente_idHidreletricas_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica).at(idCenario).getElemento(periodo);
										double								termo_independente_calculo_ENA = lista_termo_independente_calculo_ENA_x_hidreletrica_x_cenario_x_periodo.at(idHidreletrica).at(idCenario).getElemento(periodo);
								...
								//////////////////////////////////////////
								//Atualiza com as usinas OUT dentro do estudo
								//////////////////////////////////////////

								for (int pos = 1; pos <= int(lista_hidreletrica_out_estudo.size()); pos++) {

									if (lista_hidreletrica_out_estudo.at(pos).getAtributo(AttComumHidreletrica_codigo_usina_jusante_EAR, int()) == codigo_usina) {

										//Afluência natural usina a montante
										const SmartEnupla<int, IdHidreletrica>	    montante_idHidreletricas_calculo_ENA = lista_idHidreletricas_calculo_ENA_x_codigo_usina_x_cenario_x_periodo.at(lista_hidreletrica_out_estudo.at(pos).getAtributo(AttComumHidreletrica_codigo_usina, int())).at(idCenario).getElemento(periodo);
										const SmartEnupla<int, double>	montante_coeficiente_idHidreletricas_calculo_ENA = lista_coeficiente_idHidreletricas_calculo_ENA_x_codigo_usina_x_cenario_x_periodo.at(lista_hidreletrica_out_estudo.at(pos).getAtributo(AttComumHidreletrica_codigo_usina, int())).at(idCenario).getElemento(periodo);
										const double								 montante_termo_independente_calculo_ENA = lista_termo_independente_calculo_ENA_x_codigo_usina_x_cenario_x_periodo.at(lista_hidreletrica_out_estudo.at(pos).getAtributo(AttComumHidreletrica_codigo_usina, int())).at(idCenario).getElemento(periodo);
						...
						//Atualiza hidrelétrica com afluência natural atualizada
						is_atualizar_afluencia_natural.setElemento(idHidreletrica, false);
			...
			//Critério de parada
			if (is_all_afluencia_natural_atualizada)
				break;

		}//while (true) {

	}//	try {
	catch (const std::exception& erro) { throw std::invalid_argument("LeituraCEPEL::atualizar_equacionamento_afluencia_natural_x_hidreletrica: \n" + std::string(erro.what())); }
		...
		for (IdReservatorioEquivalente idReservatorioEquivalente = IdReservatorioEquivalente_1; idReservatorioEquivalente <= IdReservatorioEquivalente(maior_ONS_REE); idReservatorioEquivalente++) {
			...
			for (IdCenario idCenario = idCenario_inicial; idCenario <= idCenario_final; idCenario++) {
				...
				for (Periodo periodo = periodo_inicial; periodo <= periodo_final; horizonte_processo_estocastico.incrementarIterador(periodo)) {
					...
					///////////////////////////////////////////////////////////
					//Estrutura com a informação da componente das afluências naturais
					SmartEnupla<IdHidreletrica, double>	   coeficiente_idHidreletricas_calculo_ENA = lista_coeficiente_idHidreletricas_calculo_ENA_x_REE_x_cenario_x_periodo.at(idReservatorioEquivalente).at(idCenario).at(periodo);
					double								   termo_independente_calculo_ENA          = lista_termo_independente_calculo_ENA_x_REE_x_cenario_x_periodo.at(idReservatorioEquivalente).at(idCenario).at(periodo);
					///////////////////////////////////////////////////////////
					...
					const IdHidreletrica idHidreletrica_inicial = coeficiente_idHidreletricas_calculo_ENA.getIteradorInicial();
					const IdHidreletrica idHidreletrica_final   = coeficiente_idHidreletricas_calculo_ENA.getIteradorFinal();

					double ENA_calculada = termo_independente_calculo_ENA;

					for (IdHidreletrica idHidreletrica = idHidreletrica_inicial; idHidreletrica <= idHidreletrica_final; idHidreletrica++) {
...
void LeituraCEPEL::retorna_equacionamento_regras_afluencia_natural_x_idHidreletrica(Dados& a_dados, const IdHidreletrica a_idHidreletrica, const IdCenario a_idCenario, const Periodo a_periodo_inicial, const Periodo a_periodo, SmartEnupla<int, IdHidreletrica>& a_idHidreletricas_calculo_ENA, SmartEnupla<int, double>& a_coeficiente_idHidreletricas_calculo_ENA, double& a_termo_independente_calculo_ENA) {
	...
	try {
		...
		//Nota: Precisa de um termo independente porque existem regras operativas que dada uma condição o valor de afluência a valorar é uma constante (e.g. Belo Monte = 13900 se aflu_posto_288 >= Lim)

		const IdHidreletrica menorIdHidreletrica = a_dados.getMenorId(IdHidreletrica());
		const IdHidreletrica maiorIdHidreletrica = a_dados.getMaiorId(IdHidreletrica());

		const int codigo_posto_acoplamento_NW = lista_hidreletrica_NPOSNW.getElemento(a_idHidreletrica);
		...
		if (codigo_posto_acoplamento_NW < 0) {//Significa que é um poso natural sem regra específica (i.e. a natural valora no NW é composta por todas as incrementais das usinas a montante + a incremental da própria usina)
			...
			for (int pos = iterador_inicial; pos <= iterador_final; pos++) {
				...
				for (IdHidreletrica idHidreletrica_aux = menorIdHidreletrica; idHidreletrica_aux <= maiorIdHidreletrica; a_dados.vetorHidreletrica.incr(idHidreletrica_aux)) {
					...
					if (codigo_usina == a_dados.vetorHidreletrica.att(a_idHidreletrica).getElementoVetor(AttVetorHidreletrica_codigo_usinas_calculo_ENA, pos, int()) && codigo_usina == 169) {//Se dentro do cálculo da afluência natural aparece a usina 169-Sobradinho aponta para seu posto de acoplamento incremental codigo_posto = 169
						retorna_equacionamento_afluencia_incremental_x_posto_169(a_dados, a_idCenario, a_periodo, a_termo_independente_calculo_ENA);
						break;
					}
				...
				*/
				...
				if (afluencia_127 <= 430) {
					...
					if ((afluencia_127 - 90) > 0) {
						...
						////////////
						if (afluencia_125 <= 190)
							retorna_equacionamento_afluencia_natural_x_posto(a_dados, IdHidreletrica_Nenhum, 125, -double(119/190), a_idHidreletricas_calculo_ENA, a_coeficiente_idHidreletricas_calculo_ENA);
						else if (afluencia_125 <= 209)
				...
				if (a_periodo == a_periodo_inicial)
...
void LeituraCEPEL::imprime_produtibilidade_ENA(Dados& a_dados, std::string nomeArquivo) {
