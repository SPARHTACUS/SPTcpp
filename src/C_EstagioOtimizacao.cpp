...
bool Estagio::carregarRHSCortesBenders(const std::string a_nomeArquivo){
	...
	try {
		...
		while (lin < numLinhas) {
			...
			vetorCorteBenders.at(idCorteBenders).setVetor_forced(AttVetorCorteBenders_rhs, vetorValores);
	...
	try {
		...
		while (lin < numLinhas) {
			...
			if (getSize1Matriz(idCorteBenders, AttMatrizCorteBenders_coeficiente) == 0)
				vetorCorteBenders.at(idCorteBenders).setMatriz_forced(AttMatrizCorteBenders_coeficiente, enupla_inicializacao);
			...
			while (pos != std::string::npos) {
				...
				if (valor != "")
					vetorCorteBenders.at(idCorteBenders).setElemento(AttMatrizCorteBenders_coeficiente, idRealizacao, idVariavelEstado, atof(valor.c_str()));
...
bool Estagio::carregarEstadosCortesBenders(const std::string a_nomeArquivo){
	...
	try {
		...
		while (lin < numLinhas) {
			...
			vetorCorteBenders.at(idCorteBenders).setVetor_forced(AttVetorCorteBenders_estado, SmartEnupla<IdVariavelEstado, double>(IdVariavelEstado_1, vetorValores));
